...
def get_vol(close, lookback=100, period: str = 'hour', num_period: int = 1):
    """
    ...
    df0 = close.index.searchsorted(close.index - pd.Timedelta(period=num_period))
    df0 = df0[df0 > 0]
    df0 = (pd.Series(close.index[df0 - 1], index=close.index[close.shape[0] - df0.shape[0]:]))

    df0 = close.loc[df0.index] / close.loc[df0.array].array - 1  # freq returns
